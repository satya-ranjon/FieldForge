import { mysqlTable, varchar, timestamp, mysqlEnum, decimal, text, index, datetime } from 'drizzle-orm/mysql-core';
import { buyerProfiles, technicianProfiles } from './users.schema';

export const workOrders = mysqlTable('work_orders', {
  id: varchar('id', { length: 36 }).primaryKey(),
  buyerId: varchar('buyer_id', { length: 36 }).references(() => buyerProfiles.id).notNull(),
  assignedTechnicianId: varchar('assigned_technician_id', { length: 36 }).references(() => technicianProfiles.id),
