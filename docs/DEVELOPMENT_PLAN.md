...
`GET /users/me` works through port 3000; a `BUYER` token is rejected from a `TECHNICIAN`-only route.
...
- **RTK Query.** Add an API slice in `apps/web-buyer-portal/src/store/` pointed at
  `http://localhost:3000/api/v1`, per `RULE-FE-04`. Strip the hardcoded `initialState` fixtures —
